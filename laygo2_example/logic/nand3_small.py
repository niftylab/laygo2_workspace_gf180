...
in1 = templates['nmos180_fast_center_nf1_left'].generate(name='nm1')
in2 = templates['nmos180_fast_center_nf1_left'].generate(name='nm2')
...
ip1 = templates['pmos180_fast_center_nf1_left'].generate(name='pm1', transform='MX')
ip2 = templates['pmos180_fast_center_nf1_left'].generate(name='pm2', transform='MX')
...
dsn.place(grid=pg, inst=nbndr, mn=pg.mn.bottom_right(in2))
...
rA0= dsn.route(grid=r23, mn=_mn, via_tag=[False, False])

# B
_mn = [r12.mn(in1.pins['G0'])[0], r12.mn(ip1.pins['G0'])[0]]
dsn.route(grid=r12, mn=_mn, via_tag=[False, False])
dsn.via(grid=r12,mn=_mn[0]+[0,1])

_mn = [r23.mn(in1.pins['G0'])[0], r23.mn(ip1.pins['G0'])[0]]
dsn.route(grid=r23, mn=[_mn[0]+[0,1],_mn[0]+[1,1]],via_tag=[True,False])
rB0= dsn.route(grid=r23, mn=_mn, via_tag=[False, False])
# C
_mn = [r12.mn(in2.pins['G0'])[0], r12.mn(ip2.pins['G0'])[0]]
dsn.route(grid=r12, mn=_mn, via_tag=[True, False])

_mn = [r23.mn(in2.pins['G0'])[0], r23.mn(ip2.pins['G0'])[0]]
vC0, rC0 = dsn.route(grid=r23, mn=_mn, via_tag=[True, False])
dsn.route(grid = r23, mn=[_mn[0],_mn[0]+[-1,0]])

# OUT
_mn = [r12.mn(ip0.pins['D0'])[0], r12.mn(ip2.pins['D0'])[0]]
dsn.route(grid=r12, mn=_mn, via_tag=[True,True])

_mn = [r12.mn(ip2.pins['D0'])[0], r12.mn(in2.pins['D0'])[1],r12.mn(in2.pins['D0'])[1]-[1,0]]
vout0,rout0,vout1,rout1 = dsn.route(grid=r23, mn=_mn, via_tag=[True,True,False])
...
_mn =[r12.mn(ip0.pins['S0'])[0], r12.mn(ip2.pins['S0'])[0]]
_track = [None, r12.mn.top_left(pbndl)[1]]
dsn.route_via_track(grid=r12, mn=_mn, track=_track)

# # 6. Create pins.

pinA = dsn.pin(name='A', grid=r12, mn=r23.mn.bbox(rA0))
pinB = dsn.pin(name='B', grid=r12, mn=r23.mn.bbox(rB0))
