...
import myTemplate as myTemplate
...
def import_templates(dsn_name:str):
   isExist = False
   for name, design in lib.items():
      if name == dsn_name:
         isExist = True
         break
   if isExist == False:
      print("there's no "+dsn_name+" in the library '" + lib.name + "'")
      return None
   inst = myTemplate.import_templates(lib[dsn_name])
   return inst
