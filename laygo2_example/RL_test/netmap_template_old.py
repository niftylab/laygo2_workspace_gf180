...
#orient : bool
    def __init__(self, grid:dict, via_table:dict, orient_first="vertical",layer_names:list=['M1','M2','M3','M4','M5']):
        self.layers=dict()
    ...
    def translate_rect(self, elem, master): # translate mn of metals of (virtual)instances
        ...
        return self.grid[elem.layer[0]].mn(_xy)
    ...
    def insert_metal(self, metal):
        _layer = metal.layer[0]
        return self.layers[_layer].insert_metal(self.grid[_layer].mn(metal.xy), net_name=metal.netname)
    ...
    def insert_instance_blackbox(self, inst):
        ...
        for pin in inst.pins.values():
            ...
            _layer = pin.layer[0]
            self.layers[_layer].insert_metal(self.grid[_layer].mn(pin), net_name=pin.netname)
            pin_list.append(pin)
        return pin_list
    ...
    def insert_virtual_instance(self, vinst): 
        ...
        for velem in vinst.native_elements.values():
            ...
            # vias of instance & virtual instance are not inserted -> changed. vias are inerted
                ...
                via_list.append(viaNode(self._via_table[velem.cellname], self.grid[self._via_table[velem.cellname][0]].mn(velem.xy)))
    ...
    def insert_pin(self, pin):
        ...
        # _mn[1][0] = round(pin.xy[1][0]/72)
        # _mn[1][1] = round(pin.xy[1][1]/72)
        _mn = self.grid[_layer_name].mn(pin)
    ...
    def net_traverse(self, pin, pin_net_name,pin_set): # travel net graph in BFS order. Source node is pin
        ...
        if pin_net_name in pin_set:
            if pin.visited == False:
                _xy = self.grid[pin.layer].abs2phy(pin.mn)
                print("open error: %s [(%d %d),(%d %d)] (xy=[(%d %d),(%d %d)]) is not connected to same named net"
                      %(pin_net_name,pin.mn[0][0],pin.mn[0][1],pin.mn[1][0],pin.mn[1][1], \
                        _xy[0][0],_xy[0][1],_xy[1][0],_xy[1][1]))
            #     pin.visited = True
            else:
            #     pass
    ...
    def lvs_check(cls, dsn, grid, via_table, orient_first="vertical", layer_names=['M1','M2','M3','M4','M5']):
        ...
        for inst in dsn.instances.values():
            if nMap.is_via(inst):
            #    print(nMap._via_table[inst.cellname], nMap.grid[nMap._via_table[inst.cellname][0]].mn(inst.xy))
                nMap.vias.append(viaNode(nMap._via_table[inst.cellname], nMap.grid[nMap._via_table[inst.cellname][0]].mn(inst.xy)))
            else:
        ...
        for pin, pin_node in nMap.pins:
            print("pin name: %s, netname: %s, layer: %s, xy:[[%d %d][%d %d]], pin_node.xy:"\
                % (pin.name, pin.netname,pin.layer[0],nMap.grid[pin.layer[0]].mn(pin)[0][0],nMap.grid[pin.layer[0]].mn(pin)[0][1],nMap.grid[pin.layer[0]].mn(pin)[1][0],nMap.grid[pin.layer[0]].mn(pin)[1][1]),end='')
            print(pin_node.mn)
