...
for celltype in cell_type:
   for nf in nf_list:
      ...
      if nf==2:
         _mn = [r23.mn(ip1.pins['G'])[0]+[-1,0], r23.mn(ip1.pins['G'])[0]]
...
def import_templates(): # dsn_name:str):
   _template = myTemplate.export_templates(lib, celltype='tinv')
   return _template
...
if __name__ == "__main__":
   ...
   laygo2.interface.magic.export(lib, filename=ref_dir_MAG_exported +libname+'_'+cellname+'.tcl', cellname=None, 
                                 libpath=ref_dir_layout, scale=0.1, reset_library=False, tech_library=tech.name) 
