...
inv_sel = tlogic_prim['inv_'+str(nf_inv)+'x'].generate(name='inv_sel')
...
#    cells.append(tlogic_prim['inv_'+str(nf_inv)+'x'].generate(name='inv'+str(i)))
...
cursor = pg.mn.bottom_right(cgate)
for cell in cells:
    dsn.place(grid=pg, inst=cell, mn=cursor-abut)
...
track_sel = [None, r34.mn(inv_sel.pins['O'])[1,1]]
rsel = dsn.route_via_track(grid=r34, mn=mn_list, track=track_sel)
...
#_track = [None, r34.mn(inv_sel.pins['O'])[0,1]+1]
track_selbar = [None,track_sel[1]+1]
dsn.route_via_track(grid=r34, mn=mn_list, track=track_selbar)
