...
#   _track = [None, np.mean(r34.mn(I0[0,0].pins['SCAN_EN'])[:,1], dtype=np.int)+4]
...
#   _track = [None, np.mean(r34.mn(I0[0,0].pins['SCAN_GATE'])[:,1], dtype=np.int)+5]
...
if row * col == bit:
   for i in range(full_row-1): # one time less iteration
      _mn = [np.mean(r34.mn(cell_list[col-1 + col*i].pins['SCAN_CLK']), axis=0, dtype=np.int), r34.mn(cell_list[col + col*i].pins['SCAN_CLK_OUT'])[1]]
   ...
   for i in range(full_row):
      _mn = [np.mean(r34.mn(cell_list[col * (i+1) -1].pins['SCAN_CLK']), axis=0, dtype=np.int), r34.mn(cell_list[col * (i+1)].pins['SCAN_CLK_OUT'])[1]]
   ...
   for i in range(iter_total - full_row):
      _mn = [np.mean(r34.mn(cell_list[col-2 + full_row*col + (col-1)*i].pins['SCAN_CLK']), axis=0, dtype=np.int), r34.mn(cell_list[col-1 + full_row*col + (col-1)*i].pins['SCAN_CLK_OUT'])[1]]
...
for row_idx in range(full_row):
   for col_idx in range(col):
      ...
      end_pt_y = r45.mn.top_right(I0[row_idx,-1])[1]
      end_pt = np.asarray([end_pt_x, end_pt_y])
      _mn = np.asarray([np.mean(r45.mn(I0[row_idx, col_idx].pins['SCAN_GATE_VALUE']), axis=0, dtype=np.int)+[0,3]+[-2-col_idx,0], end_pt]) 
      vertical = dsn.route(grid=r45, mn=_mn, via_tag=[True, False])

      _mn = r34.mn(I0[row_idx, col_idx].pins['SCAN_GATE_VALUE'])
      _mn[1,1] = np.mean(_mn[:,1], dtype=np.int)+3 
...
for pin in pin_list:
   ...
   _mn[:,1] = np.mean(r34.mn.bbox(cell_eff[0].pins[pin])[:,1], dtype=np.int)
...
laygo2.interface.magic.export(lib, filename=ref_dir_MAG_exported+libname+'_'+cellname+'.tcl', libpath=ref_dir_layout, cellname=None, scale=0.1, reset_library=False, tech_library=tech.name)
