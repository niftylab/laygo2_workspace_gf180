...
for i in range(len(pg_list)):
   for j in range(len(pg_list[i])):
      if pg_list[i][j] == None:
         ...
         print( pg.mn.bbox(pg_list[i][j])[:,0], pg.mn.bbox(pg_list[i][j])[0], pg.mn.bbox(pg_list[i][j])[1])
print(nf_space)
nf_space = int( (sum(abs(nf_space[0]))-sum(abs(nf_space[1]))) / pg.mn.width_vec(tlib['space_1x'].generate(name='__dmy__'))[0] )
print(nf_space)
